...
export function LocationMap() {
            ...
            const customIcon = L.divIcon({
                ...
                html: `
                    <div style="position:relative;display:flex;align-items:center;justify-content:center;width:48px;height:48px;">
                        <div style="position:absolute;width:48px;height:48px;background:rgba(234,88,12,0.15);border-radius:50%;animation:loc-ping 2s cubic-bezier(0,0,0.2,1) infinite;"></div>
    ...
    const InfoCard = () => (
        <div className="flex flex-col justify-between h-full p-6 lg:p-8 bg-white">
            <div>
                <p className="font-mono text-[9px] uppercase tracking-[0.35em] text-accent-orange font-black mb-4">
                    04 // LOKALIZACJA
                </p>
                <h3 className="text-lg lg:text-xl font-black uppercase tracking-tight text-slate-ink leading-tight mb-1">
                    SZRAMADACH
                </h3>
                <p className="font-mono text-[10px] text-slate-ink/40 uppercase tracking-widest font-bold mb-5">
                    DEKARSTWO TARNÓW
                </p>

                <div className="space-y-3 text-sm text-slate-ink/70">
                    <div className="flex items-start gap-3">
                        <span className="font-mono text-accent-orange text-[9px] font-black uppercase mt-0.5 w-8 shrink-0">ADR</span>
                        <span className="font-medium">ul. Krakowska 123<br />33-100 Tarnów</span>
                    </div>
                    <div className="flex items-start gap-3">
                        <span className="font-mono text-accent-orange text-[9px] font-black uppercase mt-0.5 w-8 shrink-0">TEL</span>
                        <a href="tel:+48500123456" className="font-medium hover:text-accent-orange transition-colors">+48 500 123 456</a>
                    </div>
                    <div className="flex items-start gap-3">
                        <span className="font-mono text-accent-orange text-[9px] font-black uppercase mt-0.5 w-8 shrink-0">HRS</span>
    ...
    return (
        <>
            {/* MOBILE: stacked layout — card on top, map below */}
            <div className="flex flex-col lg:hidden w-full">
                <div className="border-b border-sheet-border">
                    <InfoCard />
                </div>
            ...
            {/* DESKTOP: side-by-side — card left, map right (full bleed) */}
            <div className="hidden lg:flex w-full h-[550px] relative">
                {/* Info card — fixed width, sits on the left */}
                <div className="w-[300px] shrink-0 border-r border-sheet-border z-20 h-full">
                    <InfoCard />
                </div>
                {/* Map fills the rest */}
